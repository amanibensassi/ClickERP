...
//   var url = 'http://erpdev.pingholding.com/wp-json/custom-api-route/getTache/';
//   try {
...
Future<List<Map<String, dynamic>>> fetchMeets() async {
  ...
  var url =
      'http://erpdev.pingholding.com/wp-json/custom-api-route/getReunion/?id_user=$iduser';
  try {
    var response = await http.post(Uri.parse(url));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
...
Future<List<Map<String, dynamic>>> fetchTasksID() async {
  ...
  int iduser = await getFromSession('id');
  var url =
      'http://erpdev.pingholding.com/wp-json/custom-api-route/getTache/?id_user=$iduser';
  try {
    var response = await http.post(Uri.parse(url));
    if (response.statusCode == 200) {
      final jsonData = json.decode(response.body);
